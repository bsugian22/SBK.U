...
export default function Inplay() {
    ...
    return ( <
            ...
            <
            button class = "page-left btn-0 heightp-100 padding-vertical-40 padding-horizontal-25 background-transparent-b-20 flex align-items-center justify-content-center"
            onClick = {
                () => { scrollLeft() } } >
            <
            ...
            /button> <
            div id = "scrollmenu"
            class = "heightp-100 flex align-items-center" > {
                sports ? .sports ? .data ? .map((icon, index) => {
                    ...
                    if (inplay.matches ? .data) {
                        ...
                        if (matches.length) {
                            let sportMatches = {}
                            return <a class = { inplay.sportsTypeId == icon.id ? "active" : "" }
                            key = { "icon-" + index }
                            href = "#icon"
                            onClick = {
                                    (e) => {
            ...
            button class = "page-right btn-0 heightp-100 padding-vertical-40 padding-horizontal-25 background-transparent-b-20 flex align-items-center justify-content-center"
            onClick = {
                () => { scrollRight() } } >
            <
    ...
    onClick = {
...

        {
            ...
            inplay.mainMarkets.map((match, index) => {
                    ...
                    row.push( <
                        div key = { match.id }
                        class = "title background-transparent-b-30 height-40 align-items-center-inherit" >
                        <
                        div class = "flex padding-horizontal-10 widthp-40 heightp-100" >
                        <
                        span class = "color-grey text-ellipsis" >
                        <
                        i class = "far fa-flag margin-right-5 color-yellow" > < /i> { match.tournamentId == null ? sports.tournaments ? .data ? sports.tournaments ? .data.find(x => x.id == match.simpleTournamentId) ? .tournament.name.ko : "" : sports.tournaments ? .data ? sports.tournaments ? .data.find(x => x.id == match.tournamentId) ? .tournament.name.ko : "" } <
                        /span> <
                        /div> <
                    ...
                    row.push( <
                                ...
                                if (market.status == 1) {
                                    return (market.outcomes.map((outcome, outcomes_index) => {
                                        ...
                                        return ( <
                                            ...
                                            class = {
                                                (outcome.enabled == 1 || outcome.enabled == true) && sportDetails.data.bet.outcomes.find(x => x.id == outcome.id) ? "active odd color-grey" : " odd color-grey" } >
                                            {
                                    ...
                                    return (market.outcomes.map((outcome, outcomes_index) => {
                                        return ( <
                                            div key = { "outcome_id-inactive-1x2-" + outcome.id }
                                    ...
                                    if (market.status == 1) {
                                        return (market.outcomes.map((outcome, outcomes_index) => {
                                            ...
                                            return ( <
                                                ...
                                                data - specifier = { JSON.stringify(specifer) }
                                                class = {
                                                    (outcome.enabled == 1 || outcome.enabled == true) && sportDetails.data.bet.outcomes.find(x => x.id == outcome.id) ? "active odd align-items-center-inherit justify-content-center-inherit" : " odd align-items-center-inherit justify-content-center-inherit" } >
                                                {
                                                    outcome.oldOdds == null ?
                                                    "" : outcome.oldOdds < outcome.odds ?
                                                        <
                        ...
                        <
                        div className = "flex detail widthp-100 justify-content-center align-items-center color-white"
                        class = {
                            match.id === sports.sideMarket.id ? "active flex detail widthp-100 justify-content-center align-items-center color-white" : "flex detail widthp-100 justify-content-center align-items-center color-white"
                        }
                        onClick = {
                            () => setDetail(match.id) } >
            ...
            value = {
                { label: inplay.currentPage, value: inplay.currentPage } }
            onChange = {
                (e) => {
            ...
            div class = "inplay-detail flex-inherit flex-column padding-vertical-10 padding-left-5 padding-right-10 border-left" > {
                ...
                span class = "margin-horizontal-4 color-twhite" > vs < /span> { sports.sideMarket ? .id ? sports.competitors ? .data ? sports.competitors ? .data.find(x => x.id == sports.sideMarket.awayTeamId) ? .competitor ? .name ? .ko : "" : "" } <
                /span> <
                /div> <
                ...
                i class = "far fa-stopwatch color-grey font-size-11" > < /i> {
                    sports.sideMarket ? .id ? < span class = "color-grey" > { moment(sports.sideMarket.startAt).format("MM / DD HH:mm") } < /span> : ""} { /* <span class="color-grey">{inplay.data.detail_data?.awayTeam?.name?.ko ? moment(startAt).format("MM / DD HH:mm") : ""}</span> */ } <
                        /div> <
                ...
                div class = "market-list flex-inherit flex-column scrollable-auto" > {
                    ...
                    sports.sideMarket.markets.map((market, market_index) => {
                        ...
                        rows.push( <
                            div class = "market-header height-40 background-transparent-b-30 padding-horizontal-10 align-items-center shrink-0 border-bottom-white-strong"
                            key = { "details-market-id" + market.id } >
                            <
                            span class = "color-grey" > { /* {market.marketId} */ } { setCompetitorName(marketName, homeTeam, awayTeam, specifer) } <
                            /span> <
                            /div>
                        );
                ...
                div class = "flex padding-top-5" >
                <
                span class = "color-yellow" > 1 Q 02 : 02 < /span> <
                    span class = "color-blue padding-left-5" >
                    <
                    Link to = "/inplay/detail"
                className = "color-blue" >
